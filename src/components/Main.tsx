...
//     SpeechRecognition.startListening({ continious: true, language: "en-IN" });
//   };
...
//       <Image src={googleLogo} alt="logo" height={100} width={270} />
...
const Main: React.FC = () => {
  ...
  const startListening = () => {
    SpeechRecognition.startListening({ continuous: true, language: "en-IN" }); // Corrected spelling of 'continuous'
  };
  ...
  const selectImage = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!e.target.files) return; // Check if files is null
    const file = e.target.files[0];
    if (!file) return;
    const response = await new Promise<string>((resolve, reject) => {
      // Specified Promise return type
      const reader = new FileReader();

      reader.onload = (event) => {
        if (!event.target) return;
        resolve(event.target.result as string); // Added type assertion
      };
